...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            #get distance between the thumb and index finger
            ...
            if mouseDown:
                # draw a circle at the midpoint with radius 10px
                cv2.circle(frame, (int(midpoint_x*frame_width), int(midpoint_y * frame_height)), 10, (0, 255, 0), -1)
                           
            else:
                # draw a circle at the midpoint with a radius of 10px
                cv2.circle(frame, (int(midpoint_x*frame_width), int(midpoint_y * frame_height)), 10, (0, 255, 0), 1)
